...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	newGrid.resize(height, vector <float>(width, belief_per_cell)); // Initialize newGrid
	return newGrid;
}
  ...
  float blurring) 
{
  ...
  vector < vector <float> > newGrid;
	...
	newGrid.resize(beliefs.size(), vector <float>(beliefs[0].size(), 0)); // Initialize newGrid
	int new_i, new_j;

	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			(i + dy < 0) ? new_i = (i + dy + height) % height : new_i = (i + dy) % height;
			(j + dx < 0) ? new_j = (j + dx + width) % width : new_j = (j + dx) % width;
/*
			new_i = i + dy;
			new_j = j + dx;
			if (new_i < 0) { new_i += -dy * height; } // if new_i is negative, make positive
			if (new_j < 0) { new_j += -dx * width; } // if new_j is negative, make positive
			new_i %= height;
			new_j %= width;
*/
